...
function defaultSelectedCurrency(data: Data) {
    ...
    const localCurrency = getCurrencies()
...
export async function getLocalData(): Promise<Data> {
    ...
    try {
        ...
        if (value !== null) {
            ...
            console.log("STORAGE_KEY data is null")
        }
    } catch (error) {
...
export async function setSettingsData(settings: Settings): Promise<void> {
    try {
        ...
        await setData(data);
        console.log("inside setSettingsData try: " + data.settings.size)
    } catch (error) {
        console.error("setSettingsData: " + error);
    }
}
